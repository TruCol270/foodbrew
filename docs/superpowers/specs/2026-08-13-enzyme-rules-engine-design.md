...
**Seed data:** versioned JSON in-repo (`seed/enzymes.json`, `seed/ingredients.json`, `seed/foods.json`, `seed/substrates.json`, `seed/gi_model.json`). Loaded on first boot; founder edits go to SQLite only; seeds stay pristine for reset and git-diffable review.
...
- `confirmed` — verified against a named source, recorded in the paired `*_source` field.
...
| **R11 Food-grade / GRAS** | 4l | Selected enzyme must be food-grade with GRAS status recorded. Lactase and alpha-galactosidase are largely already GRAS (a cost and time advantage). Missing or unknown → `cannot_assess` with the supplier question text. A standing banner notes that finished-product rules (food safety, acidified-food regulations) are outside this tool's scope. |
...
| Finding or gap | What the protocol asks her to do |
|---|---|
| R8 AMBER (in-jar drift predicted) | Taste checkpoints at day 0 / 3 / 7 — sweeter? off smell? separated? |
| R1 non-pass, or `measured_ph` empty | Enter a measured pH per batch (optional; unlocks the ambient storage watch when < 4.6) |
| R7 AMBER or `cannot_assess` | Log trigger food and amount per use, so the engine can report whether that meal cleared the evidence threshold |
| R4 AMBER (wet premix) | Storage watch on the schedule permitted by pH |
| **R15 envelope non-pass** | **Applied-food texture checkpoints at 0 min / 1 hr / 4 hr / overnight against a portion left undressed** |
| Any format | Make-it capture: total minutes, difficulty, what went wrong, where the enzyme went in the sequence |
| Every trial | Usability log entries per use — was a second squeeze needed, did it feel natural |
...
8. **Report** — print-friendly page: inputs, predicted findings, observed results with confidence tiers, the occasion envelope, evidence values, data gaps, sources, engine version, and a fixed footer: *"Formulation decision support. Not a safety, efficacy, or regulatory determination."*
...
- (b) Creamy dressing, recipe pH 4.4 (explicit test input, not asserted as any real food's pH), same enzymes → AMBER: R1 pass, R4 AMBER, R8 AMBER with a dairy substrate present.
...
**Report lint:** the prohibited-words list (§10) is asserted absent from every generated report and every engine message, including trial output.
