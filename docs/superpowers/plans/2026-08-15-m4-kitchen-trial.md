...
- `docs/superpowers/plans/2026-08-14-m3-variants-compare-database-report.md` — PR #3, branch `claude/m3-implementation-plan-b45eb9`. Variants, compare, database editor, proposals inbox, stale banner, report and Markdown export. **This plan is written against that branch and must be executed on top of it**, because M4 fills two sections M3 wrote as explicit absences (M3 decision #12).
...
Found by tracing §5.3's columns, §6.5's table, and §6.6's tiers against the M3 branch as written. Implement as described here; flag items 3, 4, 7, 9 and 12 to the spec owner at M4 review.
