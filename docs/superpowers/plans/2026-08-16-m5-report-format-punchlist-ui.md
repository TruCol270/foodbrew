...
- [ ] **Every M1–M4 test still passes.** M5 changes no rule and no verdict; the only pre-existing assertions that may move are the two in `tests/engine/test_report.py` that name the old `## What was checked` heading (Task 5), and they move to the new headings rather than being deleted.
